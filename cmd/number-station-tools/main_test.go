...
func TestSchedulePersistsAndRequiresExistingStation(t *testing.T) {
	...
	if len(r.data.Schedules) != 1 || r.data.Schedules[0].FrequencyHz != 5730000 {
		t.Fatalf("schedule persistence failed: %#v", r.data.Schedules)
	}
	if err := s.addSchedule(schedule{StationID: "missing", FrequencyHz: 1, StartUTC: "00:00", EndUTC: "00:01", Weekdays: []int{1}}); err == nil {
		t.Fatal("expected missing station to be rejected")
	}
}

...
func registerAnnotationHandlers(mux *http.ServeMux, rs *recordingStore) {
	...
	mux.HandleFunc("POST /api/recordings/{id}/annotations", func(w http.ResponseWriter, r *http.Request) {
		...
		if decodeJSON(r, &annotation) != nil { http.Error(w, "invalid recording annotation", http.StatusBadRequest); return }
		created, err := rs.addAnnotation(r.PathValue("id"), annotation)
		if err != nil {
			status := http.StatusBadRequest
			if err.Error() == "recording does not exist" { status = http.StatusNotFound }
	...
	mux.HandleFunc("PUT /api/recordings/{id}/annotations/{annotationID}", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			status := http.StatusBadRequest
			if err.Error() == "recording does not exist" || err.Error() == "annotation does not exist" { status = http.StatusNotFound }
			http.Error(w, err.Error(), status); return
		}
		writeJSON(w, http.StatusOK, updated)
	})
	mux.HandleFunc("DELETE /api/recordings/{id}/annotations/{annotationID}", func(w http.ResponseWriter, r *http.Request) {
		if err := rs.deleteAnnotation(r.PathValue("id"), r.PathValue("annotationID")); err != nil { http.Error(w, err.Error(), http.StatusNotFound); return }
